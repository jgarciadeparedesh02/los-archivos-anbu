...
## Promesas y el Método `.map()`

Antes de seguir, necesitamos detenernos un momento en dos conceptos que aparecen constantemente cuando trabajamos con APIs: **promesas** y el método **`.map()`**.

### ¿Qué es una Promesa?

Una **promesa** en JavaScript es un objeto que representa un valor que estará disponible en el futuro.
Ese valor puede terminar en dos estados:

* **Resuelta (fulfilled):** Todo salió bien y tenemos el resultado.
* **Rechazada (rejected):** Algo falló y tenemos un error.

Cuando usamos `fetch()`, no recibimos directamente los datos, sino una **promesa** que nos dirá *más tarde* si la petición funcionó o no.

```js
const promesa = fetch('https://api.example.com/data');

promesa
  .then(respuesta => respuesta.json()) // cuando se resuelve
  .then(datos => console.log(datos))   // procesamos los datos
  .catch(error => console.error(error)); // si falla
```

El poder de las promesas es que nos permiten **trabajar de forma asíncrona**, sin bloquear la interfaz mientras esperamos la respuesta.

---

### El Método `.map()`

Cuando recibimos datos de una API, normalmente vienen en forma de **array de objetos**.
El método `.map()` es una función de los arrays que nos permite **transformar cada elemento del array en otra cosa** y devolver un **nuevo array** con esos resultados.
...
* Devuelve un **nuevo array** con los valores transformados.
* Es muy útil para "traducir" datos de la API a fragmentos de HTML, a objetos simplificados o a cualquier otra representación.

---

### Promesas + `.map()` en Acción

Un caso típico es cuando recibimos un array de resultados desde una API y queremos convertirlos en HTML:

```js
fetch('https://jsonplaceholder.typicode.com/users')
  .then(res => res.json())
  .then(usuarios => {
    const lista = usuarios.map(user => `<li>${user.name}</li>`).join('');
...
* Cuando se resuelve, usamos `.map()` para transformar cada usuario en un `<li>`.
* Con `.join('')` unimos todas las piezas en un único string de HTML.
